...
export class Sentry {
    ...
    static nativeCrash() {
        if (Sentry.isNativeClientAvailable()) {
            Sentry.log(SentryLog.Debug, 'Sentry: call nativeCrash');
    ...
    static setExtraContext(extras) {
        Sentry.log(SentryLog.Debug, ['Sentry: call setExtraContext', extras]);
        Sentry._ravenClient.setExtraContext(extras);
        if (Sentry.isNativeClientAvailable()) Sentry._nativeClient.setExtraContext(extras);
    }
...
class NativeClient {
    ...
    captureEvent(event) {
        Sentry.log(SentryLog.Debug, ['Sentry: call captureEvent', event]);
    ...
    _activateStacktraceMerging = async() => {
        Sentry.log(SentryLog.Debug, 'Sentry: call _activateStacktraceMerging');
