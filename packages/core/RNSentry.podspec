...
Pod::Spec.new do |s|
  ...
  if use_xcframework
    ...
    # Keep Sentry's ObjC category methods from being dead-stripped at link.
    # We link Sentry statically with no whole-archive flag, so a method living
    # only in an ObjC category (what a Swift `@objc extension` compiles to, e.g.
    # `SentryReplayNetworkDetails+Capture` in sentry-cocoa 9.25/9.26, #6609) is
    # dropped and crashes at runtime with "unrecognized selector". `-ObjC` is
    # Apple's documented fix (QA1490). Unlike `-force_load` it names no archive
    # — it only affects static libs in the current link — so the same flag is
    # safe on both targets whatever the linkage (app link under static, RNSentry
    # dylib link under dynamic), pulling no second Sentry copy and needing no
    # linkage detection.
    objc_ldflags = { 'OTHER_LDFLAGS' => '$(inherited) -ObjC' }
    pod_target_xcconfig.merge!(xcframework_search_paths).merge!(objc_ldflags)
    s.user_target_xcconfig = xcframework_search_paths.merge(objc_ldflags)
  else
    raise <<~MSG
