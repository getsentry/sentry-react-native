...
public class RNSentryModuleImpl {
  ...
  protected void getSentryAndroidOptions(
    ...
    if (rnOptions.hasKey("enableLogs")) {
      ...
      if (rnOptions.getBoolean("enableLogs")) {
        // Add beforeSend to attach sentry.replay_id attribute to native logs
        options.getLogs().setBeforeSend((logEvent) -> {
          ...
          if (scope != null) {
            ...
            if (replayId != SentryId.EMPTY_ID) {
              logEvent.setAttribute("sentry.replay_id",
                  new SentryLogEventAttributeValue("string", replayId.toString()));
            }
          }
          return logEvent;
        });
