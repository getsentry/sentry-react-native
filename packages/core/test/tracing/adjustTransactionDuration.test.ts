...
describe('adjustTransactionDuration', () => {
  ...
  it('marks span as deadline_exceeded when duration exceeds maxDurationMs', () => {
    ...
    const span = startSpanManual(
      { name: 'Test Transaction', forceTransaction: true },
      span => span,
    );
  ...
  it('does not affect spans from other transactions', () => {
    const client = getClient()!;
    const trackedSpan = startSpanManual(
      { name: 'Tracked Transaction', forceTransaction: true },
      span => span,
    );
    const otherSpan = startSpanManual(
      { name: 'Other Transaction', forceTransaction: true },
      span => span,
    );
